    ...
    var body: some View {
        ...
        ZStack(alignment:.bottom){
           ...
           // ScrollView(.vertical){
            ...
            channelInfoSection1(Video:self.Video)
                   ...
                   // List{
                    ForEach(channelPlaylist.channelPlaylist_v,id: \.videoID){eachVideo in
                       ...
                       // Button(action:withAnimation{{self.startPlayingVideo(Video:eachVideo)}}){
                            VideoCellLayout(eachVideo: eachVideo,extraData:self.Video.channelImage)
            ...
            //video PLaying screen
             .sheet(isPresented: $showVideo){
                ...
                if(userCurrentScreen=="Search"){
                  //  print("IDPlay ",videoInSearch?.videoTitle)
                              videoPlayingScreen(Video:videoInSearch!)
                              }
            }
            
           // Comment Code is for a custom Presentation i made for videoplayerscreen
//            if(showVideo){
//                ZStack(alignment:.bottom){
//              //  presentation(Modal( videoPlayingScreen(Video:channelPlaylist.channelPlaylist_v[0])))
//                    videoPlayingScreen(Video:channelPlaylist.channelPlaylist_v[0])
...
//            }
        ...
        .navigationBarTitle(Text(Video.channelName),displayMode: .large)
          .navigationBarHidden(showVideo ? true : false)

    }
    
    func startPlayingVideo(Video:VideoCell){
        print("UserScreen is ",userCurrentScreen)
     if(userCurrentScreen=="WatchNow")
     {videoInWatchNow = Video}
     else if(userCurrentScreen=="Home"){videoInHome = Video}
     else if(userCurrentScreen=="Search"){
        //print("Assign to VideoinSearch in About")
        videoInSearch = Video
     //   print("IDAbout ",videoInSearch?.videoTitle)
        }
...
struct channelInfoSection1:View{
    var Video : VideoCell
     @EnvironmentObject var channelPlaylist : envClass2
    var body:some View{
